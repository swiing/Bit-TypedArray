...
for (let i = 0, bool; i < len; i++) randomArray[i] = bool = Math.random() > 0.5; // randomly
...
res = [];
randomArray.forEach(function (val, i /*, arr*/) {
  res.push([i, val]);
});
_('.forEach()', res);
...
// log properties
'constructor byteLength length'.split(' ').forEach((prop) => {
  _('.' + prop + ':', randomArray[prop]);
});
